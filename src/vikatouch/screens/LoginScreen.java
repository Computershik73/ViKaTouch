...
public class LoginScreen extends VikaScreen {
	...
	public LoginScreen() {
		...
		/*
		 * try { settingsImg = Image.createImage("/settings.png"); } catch (IOException
		 * e) { }
		 */
		pressed = false;
	...
	public void draw(Graphics g) {
		try {
			...
			if (keysMode && selectedBtn == 0) {
				...
				g.drawImage(IconsManager.ico[IconsManager.SETTINGS], DisplayUtils.width - 36,
						DisplayUtils.compact ? 0 : 8, 0);
			} else {
				g.drawImage(IconsManager.selIco[IconsManager.TOPBAR], DisplayUtils.width - 36,
						DisplayUtils.compact ? 0 : 8, 0);
			}
			// if(!shortLayout) g.fillRect(0, DisplayUtils.height-50, DisplayUtils.width,
			// 50);
			...
			// g.setFont(f);
			ColorUtils.setcolor(g, ColorUtils.BACKGROUND);
			// g.drawString("Vika Touch - "+titleLoginStr, 8,
			// (shortLayout?12:24)-f.getHeight()/2, 0);

			tapCoords = new int[] { yCenter - fH / 2 - 8 - fH, yCenter - fH / 2 - 8, yCenter - fH / 2, yCenter + fH / 2,
					yCenter + fH / 2 + 8, yCenter + fH / 2 + 8 + 32 };
			...
			ColorUtils.setcolor(g, ColorUtils.TEXTBOX_OUTLINE);
			if (loginpressed != null && (pressed || selectedBtn == 3))
				g.drawImage(loginpressed, DisplayUtils.width / 2 - login.getWidth() / 2, tapCoords[4], 0);
			else if (login != null)
				g.drawImage(login, DisplayUtils.width / 2 - login.getWidth() / 2, tapCoords[4], 0);

			f = Font.getFont(0, 0, Font.SIZE_SMALL);
			g.setFont(f);
			ColorUtils.setcolor(g, ColorUtils.TEXT); // нихера не видно ни на СЕшке, ни на ж2меЛ. Подозреваю что везде
														// не на амоледе.
			if (user != null)
