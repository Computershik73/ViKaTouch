	...
	public void sendRecord()
	{
		try {
			...
			byte[] var5 = (new HttpMultipartRequest("http://vikamobile.ru:80/upload.php?" + uploadUrl, var3, "upload_field",
					"bb2.mp3", "multipart/form-data", recorderSoundArray)).send();
			String var4 = new String(var5);
	
			String var178 = VikaUtils.downloadE(new URLBuilder("docs.save").addField("file", var4));
			String var6 = var178.substring(var178.indexOf("owner_id") + 10, var178.substring(var178.indexOf("owner_id") + 10, var178.length()).indexOf("\"") - 1);
			String var7 = var178.substring(var178.indexOf("id") + 4, var178.substring(var178.indexOf("id")+4, var178.length()).indexOf("\"") - 1);
			
			VikaUtils.downloadE(new URLBuilder("messages.send").addField("peer_id", peerId)
					.addField("attachment", "doc"+ var6 + "_" + var7));
		}
		catch (IOException e)
		{
